...
local function PauseTest()
    ...
    local function onNodeEvent(event)
        if event == "enter" then
            ...
            local  l = cc.Label:createWithTTF("After 3 seconds grossini should move", "fonts/Marker Felt.ttf", 16)
            ret:addChild(l)
...
local function RemoveTest()
    ...
    local  l = cc.Label:createWithTTF("Should not crash", "fonts/Marker Felt.ttf", 16)
    local  s = cc.Director:getInstance():getWinSize()
    ret:addChild(l)
    l:setAnchorPoint(cc.p(0.5, 0.5))
    l:setPosition( cc.p(s.width / 2, 245))
...
local function ResumeTest()
    ...
    local function onNodeEvent(event)
        if event == "enter" then
            local  l = cc.Label:createWithTTF("Grossini only rotate/scale in 3 seconds", "fonts/Marker Felt.ttf", 16)
            ret:addChild(l)
            local  s = cc.Director:getInstance():getWinSize()
            l:setAnchorPoint(cc.p(0.5, 0.5))
